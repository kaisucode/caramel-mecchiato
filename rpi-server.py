...
settings = {
	"mk6": {
		"starting_pos": {15: 120, 14: 45, 0: 160, 1: 0, 2: 0}, # servo_pin, degrees
		"ending_pos": {15: 30, 14: 0, 0: 100, 1: 60, 2: 60}, # servo_pin, degrees
		"pins": {
			"left_shoulder": 15,
			"left_arm": 14,
			"right_chest": 0,
...
def get_ending_pos(name): 
	pin = get_pin(name)
	return settings[cur_mode]["ending_pos"].get(pin, 50)
...
def run_diagnostics(): 
	...
	for i in ["left_shoulder", "left_arm", "right_chest", "right_shoulder", "right_arm"]: 
		test_high(i)
	time.sleep(1)
	for i in ["left_shoulder", "left_arm", "right_chest", "right_shoulder", "right_arm"]: 
		test_low(i)
